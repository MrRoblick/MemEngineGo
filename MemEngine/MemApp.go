...
func (m *MemApp) ReadUintPtr(Address uintptr) (uintptr, int) {
	v, s := m.ReadBytes(Address, 8)
	return BytesToUintPtr(v), s
}
...
func (m *MemApp) WriteUintPtr(Address uintptr, Value uintptr) bool {
	return m.WriteBytes(Address, UintPtrToBytes(Value))
}
